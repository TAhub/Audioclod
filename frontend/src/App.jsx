...
import { IconVinyl, IconRegistered, IconUser } from '@tabler/icons-react'
...
function App() {
  ...
  const activeViewDetails = useSelector(state => state.activeViewDetails)
  ...
  const inUserAsset = useMatch('/users/:userId/assets/:assetId')
  ...
  const userIcon = <IconUser size={16} stroke={1.5} />
  ...
  return (
    <AppShell padding="md" header={header} navbar={navbar}>
      ...
      <AppShell.Navbar>
        ...
        {inHomeAsset ? <NavLink label={activeViewDetails.name} leftSection={assetIcon} active={true} component={Link} to={inHomeAsset.pathname} /> : null}
        ...
        {inAsset ? <NavLink label={activeViewDetails.name} leftSection={assetIcon} active={true} component={Link} to={inAsset.pathname} /> : null}
        {showAdmin ? <NavLink label="Admin Console" active={inAdmin} component={Link} to="/admin" /> : null}
        {(inUser || inUserAsset) ? <NavLink label={activeViewDetails.username} leftSection={userIcon} active={true} component={Link} to={inUser?.pathname ?? ('/users/' + inUserAsset.params.userId)} /> : null}
        {inUserAsset ? <NavLink label={activeViewDetails.name} leftSection={assetIcon} active={true} component={Link} to={inUserAsset.pathname} /> : null}
      </AppShell.Navbar>
      <AppShell.Main>
        <Routes>
